...
public class BoardServiceImpl implements BoardService {
  ...
  public ActionForward getBoardList(HttpServletRequest request) {
    ...
    int total = dao.getBoardCount(); // dao로 부터 SELECT COUNT(*) 호출
    
    int display = 5; // 고정 값 사용(원하면 파라미터로 받아오는 것으로 변경도 가능함)
        
    // PageVo의 모든 정보 계산하기
    pageVo.setPaging(page, total, display);
    
    // 게시글 목록을 가져올 때 사용할 변수들 Map으로 만듬
    Map<String, Object> map = new HashMap<String, Object>();
    map.put("begin", pageVo.getBegin());
    map.put("end", pageVo.getEnd());
    
    // DB로부터 게시글 목록 가져오기
    List<BoardDto> boadrdList = dao. getBoardList(map);
    
    // 게시글 목록과 paging을 /board/list.jsp로 전달하기 위하여 request에 저장한 뒤 forward한다.
    request.setAttribute("boardList", boadrdList);
  ...
  public ActionForward getBoardByNo(HttpServletRequest request) {
    ...
    // DB로부터 게시글 가져오기
    BoardDto dto = dao.getBoardByNo(board_no);
