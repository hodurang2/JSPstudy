...
public class Upload extends HttpServlet {
     ...
     * 3. zip 압축 해제 후 lib/cos.jar 라이브러리 -> 프로젝트 webapp/WEB-INF/lib/cor.jar  
	  ...
	  MultipartRequest mr = new MultipartRequest(                    // 원래 사용하던 request
	                                request                          // 원래 사용하던 request
	                              , path                             // 파일 첨부 경로
	                              , 1024 * 1024 * 50                 // 첨부 파일의 최대 크기(50mb)
	                              , "UTF-8"                          // 인코딩
	                              , new DefaultFileRenamePolicy());  // 첨부 파일명이 중복되는 경우 첨부 파일의 이름을 바꾸는 정책
	  
	  // 요청 파라미터 (HttpServletRequest 대신 MultipartRequest mr를 이용한다.)
	  ...
	  String parent = attach.getParent(); // 첨부 파일 저장 디렉토리
	               // attach.getParent(); // 첨부 파일 저장 디렉터리
	  ...
	  out.println("<li>저장된 디렉토리: " + parent + "</li>");
	  ...
	  for(int i = 0; i < attaches.length; i++) {
	    out.println("<div><a href=\"/servlet/download?path="+ URLEncoder.encode(attaches[i].getPath()) +"\">" + attaches[i].getName() + "</a></div>");
	  }
