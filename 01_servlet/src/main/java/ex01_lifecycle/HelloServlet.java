 ...
 * 2. 클라이언트의 요청을 받을 수 있느 Java 클래스이다.
 * 3. javax.servlet.http.HttpServlet 클래스를 상속 받는다.
 * 4. jsp/servlet container(톰캣)에 저장되기 때문에 동일한 이름의 서블릿을 2개 이상 만들수없다. 
...
public class HelloServlet extends HttpServlet {
  ...
  public HelloServlet() {
	 ...
	 *   2) 클라이언트(HTML)로부터 요청을 받을 수 있고, 클라이언트(HTML)에게 응답할 수 있다.
	 ...
	 *   3) POST 방식으로 요청하는 방법
   *     (1) <form method="psot" action="/servlet/HelloServlet">
   *     (2) $.ajax({
   *           type: 'post',
   *           url: '/servlet/HelloServlet',
   *           ...
